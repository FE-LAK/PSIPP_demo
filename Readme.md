# Priority-based Safe Interval Path Planning - map and results repository
...
```
where `i` indicates the task index, `N` number of agents. Each line also contains `N` start and `N` goal nodes.
...
### Chained tasks scenarios

The file has the following structure
```
<i>;<N>;<list of nodes>
```
where `i` indicates the task index, `N` number of agents. Each line also contains `N` nodes. First line defines starting nodes, while each successive line defines a set of new goal nodes.
...
<i>;<N>;<list of starts>;<list of goals>
```
where `i` indicates the task index, `N` number of agents. First line contains the list of `N` start nodes, while additional lines contain `N` pick-up and `N` drop nodes.
...
### Chained and pick&drop results format

The results file of chained and pick&drop results only contain total results for PSIPPl and CCBS algorithms.